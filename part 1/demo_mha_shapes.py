...
OUT_TXT = os.path.join(os.path.dirname(__file__), 'out', 'mha_shapes.text')
...
if __name__ == "__main__":
    ...
    log(f"Input x:          {tuple(x.shape)} = (B,Td_model)")
    qkv = attn.qkv(x) # (B,T,3*d_model)
    log(f"Linear qkv(x):    {tuple(qkv.shape)} = (B,T,3*d_model)")


    qkv = qkv.view(B, T, 3, n_head, d_head)
    log(f"view to 5D:    {tuple(qkv.shape)} = (B,T,3,n_head,d_head)")

    q, k, v = qkv.unbind(dim=2)
    log(f"q, k, v split:       q={tuple(q.shape)} k={tuple(k.shape)} v={tuple(v.shape)} = (B,T,n_head,d_head)")

    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
    log(f"transpose heads: q={tuple(q.shape)} k={tuple(k.shape)} v={tuple(v.shape)} = (B,n_head,T,d_head)")
    ...
    log(f"softmax:       {tuple(weights.shape)} = (B,n_head,T,T)")

    ctx = torch.matmul(weights, v)
    log(f"context @v:   {tuple(ctx.shape)} = (B,n_head,T,d_head)")
