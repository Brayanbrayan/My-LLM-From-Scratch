...
def compute_reward(reward_model: RewardModel, tok: RLHFTokenizer, prompt_text: str, response_ids: list[int], device) -> float:
    #Build full formatted text (as in the repo)
...
def main():
    ...
    #small prompt pool (reuse the helper)
    ...
    while step < args.steps:
        ...
        for i, (ids, bnd) in enumerate(zip(seq_list, boundary_list)):
            ...
            for pid in range(P):
                idxs = [i for i in range(8) if prompt_id_of[i] == pid]
                if not idxs:
                    continue
            ...
            traj_adv = raw_rewards_t - group_mean #(8,)
            ...
            #Normalize advanatages (optional but helpful)
            if adv_flat.numel() > 1:
