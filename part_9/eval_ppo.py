...
def score_policy(policy_ckpt: str, rm_ckpt: str, bpe_dir: str | None, n: int = 16):
    ...
    prompts = sample_prompts(n) # Fixed typo: smaple -> sample
    rewards = []
    
    print(f"\n--- Evaluating {n} prompts ---")
    ...
    for i, p in enumerate(prompts):
        ...
        # FIX: Slice using 'y' for current policy, not 'y_old'
        ...
        print(f"[{i+1}/{n}] Reward: {r:.4f} | Prompt: {p[:50]}...")

    return sum(rewards)/max(1, len(rewards))

if __name__ == '__main__':
    p = argparse.ArgumentParser()
    # Fixed triple dashes to double dashes
    ...
    print(f"\n✅ Final Average RM reward: {avg_r:.4f}")