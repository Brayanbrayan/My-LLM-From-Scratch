...
def _log_params_tb(logger, args, total_steps):
    ...
    try:
        h = dict(
            ...
            warmup_steps=args.warmup_steps, batch_size=args.batch_size, grad_accum=args.grad.grad_accum_steps,
            mixed_precision = args.mixed_precision, steps=args.steps, epoch=args.epochs,
        )
...
def _log_model_stats(logger, model, step: int, do_hists: bool = False):
    ...
    try:
        ...
        if grads:
            total_grad_norms = torch.norm(torch.stack([g.detach().norm(2) for g in grads]), 2).item()
...
def save_checkpoint(model, optimizer, scheduler, amp, step: int, out_dir: str,
    ...
    # Prefer the models own config if available (e.g., a dict or dataclass with __dict__/asdict)
...
def load_checkpoint(model, path: str, optimizer=None, scheduler=None, amp=None, strict: bool = True):
    ...
    if cfg: 
        ...
        # Legacy checkpoint without config: stongly encourage a rebuild step elsewhere
...
def atomic_save_all(model, optim, sched, amp, step: int, out_dir: Path,
    ...
    save_checkpoint(model, optim, sched, amp, step, str(out_dir), tok_dir, config=config) # write model_last.pt
